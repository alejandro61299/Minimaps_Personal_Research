...
bool M_Map::PostUpdate(float dt)
{
	...
	SDL_RenderSetClipRect(app->render->renderer, &(*app->render->cameras.begin())->screen_section);

	std::vector<Tile>aux = data.quad_tree->GetTilesIntersection(*(*app->render->cameras.begin()));
		...
		if (layer->visible)
		{
			...
			for (pugi::xml_node tile = layer_data.child("tile"); tile; tile = tile.next_sibling("tile"))
			{
				...
				if (qtile.id != 0)
				{
					...
					data.quad_tree->InsertTile(qtile);
				}
				++i;
			}
		}
...
bool M_Map::LoadMap()
{
	...
	else
	{
		...
		uint level = 1;
		LOG("%f", (*app->render->cameras.begin())->screen_section.w*0.25F);
		data.quad_tree->ReturnNumbreOfLevels(area.w, (*app->render->cameras.begin())->screen_section.w*0.25, level);
		data.quad_tree = new Quadtree_Map(area, 0, level);
		level = 1;
	}
...
fPoint M_Map::MapToCamera(const fPoint map_pos, const Camera* camera)
{
	...
	camera_pos.x = (map_pos.x - map_pos.y) * (float)(data.tile_width * 0.5f) - (float)camera->rect.x + (float)camera->screen_section.x;
	camera_pos.y = (map_pos.x + map_pos.y) * (float)(data.tile_height * 0.5f) - (float)camera->rect.y + (float)camera->screen_section.y;
...
bool M_Map::Unload()
{
	...
	if (data.quad_tree != nullptr)
	{
		delete data.quad_tree;
		data.quad_tree = nullptr;
	}
